...
export default function Home() {
  ...
  const presaleMint = async () => {
    try {
      ...
      const tx = await nftContract.presaleMint({
        // value signifies the cost of one chain dev which is "0.01" eth.
      ...
      await tx.wait();
      setLoading(false);
      window.alert("You successfully minted a Chain Dev!");
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const publicMint = async () => {
    try {
      ...
      // update methods
      const nftContract = new Contract(NFT_CONTRACT_ADDRESS, abi, signer);
      // call the mint from the contract to mint the Chain Dev
    ...
    <div>
      ...
      <div className={styles.main}>
        <div>
          ...
          <div className={styles.description}>
            Its an NFT collection for developers in Chain.
          </div>
          <div className={styles.description}>
        ...
        <div>
          <img className={styles.image} src="./ChainDevsITU/ITU_3.jpg" />
        </div>
      </div>
